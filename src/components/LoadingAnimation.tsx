...
export function LoadingAnimation({ theme }: { theme: Theme }) {
  ...
  return (
    <div className="flex flex-col items-center justify-center space-y-4 py-4">
        ...
        <div className="grid gap-[1px]" style={{
          ...
          {fullMatrix.map((row, rowIndex) => 
            row.map((pixel, colIndex) => {
              ...
              let bgColor = isBackground ? 'transparent' : (isEye ? colors.slimeEye : colors.slimeBody);
      ...
      {/* Loading Text */}
      <div className="font-['Press_Start_2P'] text-[8px] text-center" style={{ color: colors.text }}>
        Loading...
      </div>
    </div>
  );
